module VersionFu
  ...
  module ClassMethods
    def version_fu(options={}, &block)
      ...
      attr_accessor  :without_new_version
      ...
      const_set(versioned_class_name, Class.new(ActiveRecord::Base)).class_eval do
        ...
        def current?
          version == send(original_class.name.underscore).send(version_column)
        end
        
        # Pass any unknown methods to the parent object
        def method_missing(method, *args)
          return super if respond_to?(method)
          send(original_class.name.underscore.to_sym).send(method, *args)
        end
      end
      ...
      versioned_class.belongs_to self.name.underscore.to_sym, 
  ...
  module InstanceMethods
    ...
    def revert_to(version)
      unless version.to_i == self.version
        return false unless v = find_version(version)
        @not_current = true
        self.version = version
        self.without_new_version = true
        versioned_columns.each do |attr|
          self.__send__(:"#{attr}=", v.send(:"#{attr}"))
        end
#        self.attributes = versioned_columns.inject({ :version => version, :without_new_version => true }) do |attrs,a|
#          attrs[a] = v.__send__ a
#          attrs
#        end
      end
      self
    ...
    def revert_to!(version)
      return false unless revert_to(version)
      @not_current = save
    end
    ...
    def create_new_version?
      return false if without_new_version
    ...
    def instatiate_revision
      ...
      version_number = (new_record? || versions.latest.blank?) ? 1 : versions.latest.version + 1
